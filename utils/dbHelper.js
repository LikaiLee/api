...
const { hyphenToHump, humpToHyphen } = require('./utils');
...
const handleResults = (error, results) => {
    if (results) {
        return { results, success: true, error: null, statusCode: 1 }
    } else {
        return { results, success: false, error, statusCode: 0 };
    }
}
/*================================== export ===========================================*/
/**
 ...
 * @return {[Object]}                            
 */
const findByConditions = ({ tableName, fields, where: whereArr, order, group }) => {
    return new Promise( async resolve => {
        // 获取别名
 ...
 * @return {[Object]}                      
 */
const addRecord = ({ tableName, values }) => {
    return new Promise( async resolve => {
        if (!tableName || !values) {
            resolve(handleResults('参数错误！', null));
        } else {
 ...
 * @return {[Object]}                               
 */
const updateRecord = ({ tableName, values, where: whereArr }) => {
    return new Promise(async resolve => {
 ...
 * @return {[type]}                  
 */
const deleteRecord = ({ tableName, where: whereArr }) => {
    return new Promise(async resolve => {
...
const _parseFieldsArrToAliasSql = (arr) => {
    ...
    const asSql = t.replace(/\w+,/g, match => {
        const field = match.substring(0, match.length - 1);
        return field + " AS " + hyphenToHump(field) + ", ";
    });
    // id AS id, stu_id AS stuId, password AS password//
